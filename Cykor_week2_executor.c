...
int execute_pipe(char** cmd_args, ChunkInfo *pipe_chunk, int pipe_count, int is_background){
    ...
    TokenInfo *pipe_tokens = malloc(sizeof(TokenInfo)*10);
    int status = 0;
...
int command_branch(char **command_chunk, ChunkInfo *chunk, int chunk_count, int chunk_num) {
    ...
    char**token_array;
    int is_background = 0;
    TokenInfo *tokens = malloc(sizeof(TokenInfo)*10);
    ...
    if(scan_tokens(command_chunk[i], &tokens, &token_count)){
        ...
        int command_count = 0;
        int pipe_count = 0;
        char**pipe_array = NULL; //안전하게 NULL로 초기화화
        ChunkInfo *pipe_chunk = malloc(sizeof(ChunkInfo)*10);;
