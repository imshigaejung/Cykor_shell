...
int main(){
    ...
    //명령어 입력 & 개행문자 제거
        ...
        scan_tokens(input, tokens, &token_count);

        //스캔한 문자열을 토큰화 시켜서 parsed_array에 저장
        parsed_array = build_token_array(input, tokens, token_count);

        //이제 input에 할당 되었던 메모리 다시 해제
        free(input);
        
        //명령어를 내장 명령어, 외부 명령어, 지시 사항으로 분리
        scan_command(parsed_array, tokens, token_count);

        //토큰들을 명령어 처리 함수로 넘김
        execute_command(parsed_array, tokens, token_count);

        //입력된 문자열이 잘 파싱되었는지 확인 차 출력
        /*printf("Parsed Array:\n");
        for (int i = 0; parsed_array[i] != NULL; i++) {
        printf("  [%d]: %s - %s\n", i, parsed_array[i], token_type_to_str(tokens[i].type));
...
const char* token_type_to_str(TokenType type) {
    switch(type) {
        ...
        case CMD_INTERNAL: return "CMD_INTERNAL";
